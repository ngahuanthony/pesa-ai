...
function validateSignupInput({ businessName, category, personalPhone, pesaAiNumber, email, password, consent }) {
  if (!businessName || !category || !personalPhone || !pesaAiNumber || !email || !password) {
    throw db.httpError(400, "businessName, category, personalPhone, pesaAiNumber, email and password are all required");
  }
...
function signup({ body }) {
  const { businessName, category, businessPhone, personalPhone, pesaAiNumber, paybillNumber, fullName, buildingName, stallNumber, publicPhone, nationalId, email, password, consent, plan } = body || {};
  const normalizedPersonalPhone = personalPhone || businessPhone;
  validateSignupInput({ businessName, category, personalPhone: normalizedPersonalPhone, pesaAiNumber, email, password, consent });

  const { passwordHash, passwordSalt } = auth.hashPassword(password);

  const { business, account } = db.mutate((state) => {
    const business = db.createBusiness(state, { name: businessName, category, phone: normalizedPersonalPhone || "", personalPhone: normalizedPersonalPhone, pesaAiNumber, paybillNumber, ownerName: fullName, buildingName, shopNumber: stallNumber, publicPhone, idOrKraPin: nationalId, plan });
