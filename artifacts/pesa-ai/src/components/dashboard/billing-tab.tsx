...
import { CreditCard, Zap, CheckCircle2, Smartphone, Calendar, ChevronRight } from "lucide-react";
...
const statusBadge = (s: string) => {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${map[s] ?? "bg-muted text-muted-foreground"}`}>
      {s.replace("_", " ")}
    </span>
  );
};
...
export function BillingTab() {
  ...
  const handlePlanChange = (planName: string) => {
    ...
    changePlan.mutate({ businessId, data: { plan: planName } }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getGetSubscriptionQueryKey(businessId) });
        toast({ title: `Switched to ${planName} plan` });
      },
    });
  };
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {PLANS.map((plan) => {
            ...
            const isUpgrade = (sub?.priceKES ?? 0) < plan.priceKES;
            return (
              <div
                key={plan.name}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <p className={`font-bold ${plan.highlighted ? "text-primary" : "text-foreground"}`}>{plan.name}</p>
                ...
                {!isCurrent && (
                  <button
                    ...
                    className={`w-full h-9 rounded-xl text-sm font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-1 ${
                      plan.highlighted
