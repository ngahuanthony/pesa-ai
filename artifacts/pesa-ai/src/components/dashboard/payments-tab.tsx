  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="space-y-3 rounded-xl border border-border p-4">
        <div><h3 className="text-sm font-semibold">Safaricom Daraja credentials</h3><p className="mt-1 text-xs text-muted-foreground">Create a Lipa na M-Pesa Online app at developer.safaricom.co.ke. These values are encrypted and are never shown again after saving.</p></div>
        <div><label className="text-sm font-medium">Consumer Key</label><Input className="mt-1.5" value={consumerKey} onChange={(e) => setConsumerKey(e.target.value)} placeholder="Daraja consumer key" autoComplete="off" /></div>
        <div><label className="text-sm font-medium">Consumer Secret</label><Input className="mt-1.5" type="password" value={consumerSecret} onChange={(e) => setConsumerSecret(e.target.value)} placeholder="Daraja consumer secret" autoComplete="new-password" /></div>
        <div><label className="text-sm font-medium">STK Push Passkey</label><Input className="mt-1.5" type="password" value={passkey} onChange={(e) => setPasskey(e.target.value)} placeholder="Lipa na M-Pesa passkey" autoComplete="new-password" /></div>
      </div>
