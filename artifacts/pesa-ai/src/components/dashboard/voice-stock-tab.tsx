...
function normalizedProductText(value: unknown) {
  return String(value || "")
    .toLowerCase()
    .replace(/\bmarks?\b/g, "max")
    .replace(/\bmaks\b/g, "max")
    .replace(/[^\p{L}\p{N}]+/gu, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
export function VoiceStockTab() {
    ...
    interpret.mutate({ businessId, data: { transcript: text.trim() } }, {
      onSuccess: (result) => {
        ...
        const nextRows = result.items.map((item) => {
           const existing = item.productId ? products.find((product) => product.id === item.productId) : undefined;
           const existingVariant = existing?.colorStock?.find((variant) => variant.color.toLowerCase() === (item.color || "").toLowerCase());
           return { ...item, id: crypto.randomUUID(), productName: existing?.name || item.productName, imageUrl: existingVariant?.imageUrl || null };
         });
  ...
  const invalid = (draft ?? []).some((item) => item.confidenceLevel === "blocked" || !item.productName || item.productName.trim().length < 3 || !item.action || !item.quantity || item.quantity <= 0 || item.quantity > 100000 || (item.productId && (previews.get(item.id)?.next ?? 0) < 0));
  const confirmAll = () => {
    if (!draft || invalid || confirming || !businessId) return;
    confirm.mutate({ businessId, data: { transcript, requestId: confirmationRequestId.current, rawTranscript: transcript, clientRequestId: confirmationRequestId.current, parserVersion: "v2.1", items: draft.map((item) => ({ productId: item.productId || null, productName: item.productName || "", action: item.action as VoiceStockConfirmInputItemsItemAction, quantity: item.quantity!, unit: item.unit, color: item.color || null, size: item.size || null, imageUrl: item.imageUrl || null })) } }, {
      onSuccess: () => {
  ...
  return <main className="mx-auto min-h-[100dvh] max-w-5xl space-y-5 px-3 py-4 sm:px-6 sm:py-7">
    ...
    <section className="overflow-hidden rounded-2xl border border-primary/15 bg-card shadow-[0_12px_35px_-25px_hsl(var(--primary))]">
      <div className="border-b border-border bg-primary/[0.045] p-4 sm:p-5">
        ...
        {(transcribe.isPending || interpret.isPending) && <div data-testid="status-processing" className="mt-3 flex items-center gap-2 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-medium text-sky-900"><Loader2 className="h-4 w-4 animate-spin" />{transcribe.isPending ? "Transcribing your recording…" : "Preparing stock changes for review…"}</div>}
         <div className="mt-4"><label htmlFor="voice-transcript" className="mb-2 block text-xs font-bold uppercase tracking-wider text-muted-foreground">Transcript · editable batch input</label><Textarea id="voice-transcript" data-testid="input-voice-transcript" value={transcript} onChange={(event) => setTranscript(event.target.value)} placeholder={"One movement per line, for example:\n16 Pro Max clear cover black 10 pieces\nSamsung Fold 5 blue 5 pieces\nreceived 12 packets of milk"} className="min-h-28 resize-y bg-background/70 text-sm" disabled={recording || transcribe.isPending} /><p className="mt-2 text-xs text-muted-foreground">Paste or dictate as many stock lines as needed. Every line is reviewed together before anything is applied.</p><div className="mt-3 flex flex-wrap justify-end gap-2"><Button data-testid="button-analyze-transcript" onClick={() => interpretText(transcript)} disabled={!transcript.trim() || recording || interpret.isPending} className="gap-2">{interpret.isPending && <Loader2 className="h-4 w-4 animate-spin" />}Prepare review</Button><Button data-testid="button-reset-voice-stock" variant="ghost" onClick={() => { stopRecording(); setDraft(null); setTranscript(""); setMicError(""); }} disabled={!transcript && !draft} className="gap-2"><RotateCcw className="h-4 w-4" />Clear</Button></div></div>
    ...
    {draft && <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
       ...
       {invalid && <div data-testid="status-review-blocked" className="flex items-center gap-2 border-t border-amber-200 bg-amber-50 px-4 py-3 text-xs font-medium text-amber-900"><AlertTriangle className="h-4 w-4" />Add a product name, action, and quantity before confirming.</div>}
    </section>}
